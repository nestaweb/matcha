...
export async function POST(req: NextRequest) {
	...
	try {
		const { encryptedUserId, newEmail } = await req.json();
		...
		const modifyVerified = await pool.query(
			'UPDATE users SET verified = false WHERE id = $1 RETURNING *',
			[userId]
		);

		if (modifyVerified.rows.length === 0) {
			return NextResponse.json({ error: 'User does not exist' }, { status: 404 });
		}
		...
		const mailer = await transporter.sendMail({
			...
			to: newEmail,
			subject: `Hello ${modifiedUser.rows[0].firstname} !`,
			text: JSON.stringify(message), 
			html: emailHtml
		})

		const logout = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/users/logout`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json'
			},
		...
		if (logout.status !== 200) {
	 	...
	 	return NextResponse.redirect(new URL(`${process.env.NEXT_PUBLIC_BASE_URL}/verifyEmail?userId=${keyEncryptedUserId.encryptedText}.${keyEncryptedUserId.iv}`, req.url));
  	}
	catch (error: any) {
