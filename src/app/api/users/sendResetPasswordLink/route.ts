...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const reEncryptedEmail = cryptoService.encrypt(userExists.email.toString());
		const encryptedParamEmail = reEncryptedEmail.encryptedText + '.' + reEncryptedEmail.iv;
		...
		const mailer = await transporter.sendMail({
			...
			to: userExists.email,
			subject: `Reset Your Password ${userExists.firstname} !`,
			text: JSON.stringify(message), 
			html: emailHtml
		})

		console.log('Email sent:', message);
		return NextResponse.json(encryptedParamEmail, { status: 200 });
