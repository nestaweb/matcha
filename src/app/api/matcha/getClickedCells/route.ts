...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const clickedCells = await pool.query(
			'SELECT * FROM matcha_clicked_cells WHERE grid_id = $1',
			[gridId]
		);

		console.log('Query successful:', clickedCells.rows);
		return NextResponse.json(clickedCells.rows, { status: 200 });
  	}
	catch (error: any) {
