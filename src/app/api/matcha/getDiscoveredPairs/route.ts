...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const discoveredPairs = await pool.query(
			'SELECT * FROM matcha_pairs WHERE grid_id = $1 AND discovered = true',
			[gridId]
		);

		console.log('Query successful:', discoveredPairs.rows);
		return NextResponse.json(discoveredPairs.rows, { status: 200 });
  	}
	catch (error: any) {
